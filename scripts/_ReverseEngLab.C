...
void _ReverseEngLab(string fPath, string oFN = "_wf_out.root")
{
  ...
  while(!checkEndOfFile)
  {
    ...
    {
      cout << "events processed -----> \t" << c.eventCounter<<"/"<<c.t_nentries 
           << "\t\r" << std::flush;
    }
    // grab entry
    bool noHeader = false;
    ...
    /*if(c.eventCounter ==0 || c.eventCounter == 1||c.eventCounter == c.t_nentries-1||c.eventCounter == c.t_nentries -1)
    {
      cout<<"EVENT DRAWN"<<"event number "<<c.eventCounter<<endl;
      new TCanvas;
      TH1F *hwfV1 = (TH1F*) c.m_wp->hwfV->Clone(p);
      hwfV1->Draw();
      hwfV1->SetDirectory(0);
    }*/
    //TH1F *hwfV1 = (TH1F*) c.m_wp->hwfV->Clone(p);
    //hwfV1->Write();
