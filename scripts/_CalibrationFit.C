...
int _CalibrationFit()
{
  ...
  //func->SetParLimits(2,0,1);
	points->Fit(func,"r");
        //set x axis title to ADC
        //set y axis title to lo (MeVee)
        //set title to ADC to lo Conversion graph
        points->GetXaxis()->SetTitle("ADC");
        points->GetYaxis()->SetTitle("lo (MeVee)");
        points->SetTitle("ADC to lo Conversion graph");
	points->Draw("AP*");
